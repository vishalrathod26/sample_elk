...
// Purpose: Delete custom roles and role mappings
//
// Node requirements:
...
//
// Run script
//     $ node delete_rbac_custom.js <rbac-custom.csv> <environment>
//		 environment:
...
if (process.env.ELASTIC_ENV_PATH == null) {
	...
	console.log("1. at the command prompt type: ELASTIC_ENV_PATH=<path to .env file> node delete_rbac_custom.js <rbac-custom-filename> <environment>")
	...
	console.log("In the 2 scenario, at the command prompt, just do:  node delete_rbac_custom.js <rbac-custom-filename> <environment> ")
    process.exit(1)
}

if (process.argv.length != 4) {
	console.log("usage: node delete_rbac_custom.js <rbac-custom-filename> <environment>")
    process.exit(2)
}
