...
//
// Run script
//     $ node create_kibana_roles.js <kibana-roles-filename> <environment>
//       environment:
...
if (process.env.ELASTIC_ENV_PATH == null) {
	...
	console.log("1. at the command prompt type: ELASTIC_ENV_PATH=<path to .env file> node create_kibana_roles.js <kibana-roles-filename> <environment>")
	...
	console.log("In the 2 scenario, at the command prompt, just do:  node create_kibana_roles.js <kibana-roles-filename> <environment> ")
    process.exit(1)
}

if (process.argv.length != 4) {
	console.log("usage: node create_kibana_roles.js <kibana-roles-filename> <environment>")
    process.exit(2)
}
...
for (var i = 1; i < rows.length; i++) {
	...
	create_kibana_roles(rows[i])
}

function create_kibana_roles(row) {
	try {
		var kibana_role_file = fs.readFileSync(row, 'utf8')
	} catch (err) {
		console.log(err)
		process.exit(5)
	}
	...
	try {
		kibana_role_obj = JSON.parse(kibana_role_file)
	} catch(err) {
		console.log(err)
		process.exit(6)
	}
	...
	update_elastic(api_url, kibana_role_obj)
}
