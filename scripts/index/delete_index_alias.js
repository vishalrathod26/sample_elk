...
if (process.env.ELASTIC_ENV_PATH == null) {
	...
	console.log("1. at the command prompt type: ELASTIC_ENV_PATH=<path to .env file> node delete_index_alias.js <index-list-filename> <org-space-filename> <environment>")
	...
	console.log("In the 2 scenario, at the command prompt, just do:  node delete_index_alias.js <index-list-filename> <org-space-filename> <environment> ")
    process.exit(1)
}

if (process.argv.length != 5) {
	console.log("usage: node delete_index_alias.js <index-list-filename> <org-space-filename> <environment>")
    process.exit(2)
}
...
function create_index(row) {
	...
	for (i = 0; i < index_list.length; i++) {
		api_url = base_index_api_url + index_list[i] + '-' + cols[co] + '-000001/' + "_alias" + "/" + index_list[i] + "-" + cols[co]
		console.log(api_url)
