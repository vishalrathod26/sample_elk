...
//
// Run script
//     $ node create_index_alias.js <index-list-filename> <org-space-fiename> <environment>
//       environment:
...
if (process.env.ELASTIC_ENV_PATH == null) {
	...
	console.log("1. at the command prompt type: ELASTIC_ENV_PATH=<path to .env file> node create_index_alias.js <index-list-filename> <org-space-filename> <environment>")
	...
	console.log("In the 2 scenario, at the command prompt, just do:  node create_index_alias.js <index-list-filename> <org-space-filename> <environment> ")
    process.exit(1)
}


if (process.argv.length != 5) {
	console.log("usage: node create_index_alias.js <index-list-filename> <org-space-filename> <environment>")
    process.exit(2)
}
...
// -----------------------------------------------------------------------------
// Creata a reuasble JON object represent an index alias
// -----------------------------------------------------------------------------
index_alias_entry = '{ ' +
	'"actions": [ { "add": { "index": "index-pattern-goes-here", "alias": "alias-goes-here", "is_write_index": true }} ]' +
'}';


try {
	var index_alias_obj = JSON.parse(index_alias_entry)
} catch(err) {
	console.log(err)
	process.exit(5)
}
...
function create_index(row) {
	...
	for (i = 0; i < index_list.length; i++) {
		api_url = base_index_api_url + "_aliases"
		index_alias_obj.actions[0].add.index = index_list[i] + "-" + cols[co] + "-*"
		index_alias_obj.actions[0].add.alias = index_list[i] + "-" + cols[co]
		console.log(api_url)
		console.log(JSON.stringify(index_alias_obj))
		update_elastic(api_url, index_alias_obj, elastic_username, elastic_password)
	}
}
